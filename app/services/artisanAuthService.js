const model = require("../models/artisan");
...
const artisanRepo = new baseRepo(model)
...
exports.ArtisanRegistrationToken = option => {
...
exports.verifyArtisan = (publicId, option) => {
  return new Promise((resolve, reject) => {
    model
      .findOne({ publicId: publicId })
      .then(found => {

        if (found.userType == 'artisan') {
...
exports.completeArtisanSignup = (publicId, data) => {
  return new Promise((resolve, reject) => {
        ...
        getUserDetail(publicId).then(activeUser => {
          generateToken(activeUser).then(token => {
            resolve({
              ...
              message: 'artisan profile updated successfull !!!'
            })
          }).catch(err => reject(err))
        }).catch(err => reject(err))
      } else {
...
exports.updateProfile = function (id, data) {
  return new Promise((resolve, reject) => {
    artisanRepo.updateByQuery({ publicId: id }, data)
      .then(updated => {
        if (updated) {
          artisanRepo.getById(updated._id)
            .then(user =>
              resolve({
                success: true,
                data: user,
